...
f_dw1dt = sm.lambdify((t, g, k, L0, m1, m2, m3, l1, l2,
                      theta1, theta2, dtheta1, dtheta2), sols[d2theta1])
f_dtheta2 = sm.lambdify(dtheta2, dtheta2)
f_dw2dt = sm.lambdify((t, g, k, L0, m1, m2, m3, l1, l2,
                      theta1, theta2, dtheta1, dtheta2), sols[d2theta2])
...
x1, y1 = (l1*np.cos(theta1_sol), -l1*np.sin(theta1_sol))
x2, y2 = (2*l1*np.cos(theta1_sol),  np.zeros(len(theta1_sol)))
x3, y3 = (2*l1*np.cos(theta1_sol) + l2 *
          np.sin(theta2_sol), -l2*np.cos(theta2_sol))
...
ani = animation.FuncAnimation(
    fig, animate, frames=1000, interval=50, repeat=False)
